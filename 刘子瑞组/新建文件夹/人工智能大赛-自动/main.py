...
def box_go1(n):    #抱着箱子前进一步
    for i in range (0,n):
        base_action.action("抱块前进v1_1步")
        time.sleep(0.5)
def box_go2(n):     #抱着箱子前进两步
    for i in range (0,n):
        base_action.action("抱块前进v1_2步")
        time.sleep(0.5)
def box_go3(n):     #抱着箱子前进三步
    for i in range (0,n):
        base_action.action("抱块前进v1_3步")
        time.sleep(0.5)
...
def goto_box():
    ...
    if chest_circle_x is None :
        ...
        if chest_circle_x < 260:
            ...
            if chest_circle_y < 340:  
                print("前进",chest_circle_y)
                base_action.action("FastForward1s")
                time.sleep(0.5)
            elif chest_circle_y >= 420:  
...
def turn_to_tag(dis_x, dis_y, theta, x_offset=0, y_offset=0, theta_offset=0):
    # 调整后的阈值参数
    x_threshold = 0.04    # x方向误差阈值 (原0.03)
    y_threshold = 0.03    # y方向误差阈值 (原0.02)
    theta_threshold = 10  # 角度误差阈值 (原5)
    large_angle_threshold = 30  # 大角度调整阈值
    ...
    print(f"ID:{marker[0]} 误差[x:{x_error:.3f}, y:{y_error:.3f}, θ:{theta_error:.1f}°]")

    # 分层控制逻辑
    if step == 1:
        # 第一层：大角度纠偏（优先级最高）
        if abs(theta_error) > large_angle_threshold:
            if theta_error > 0:
                print(f"大角度左转{theta_error:.1f}°>{large_angle_threshold}°")
                BoxL_turn2(1)
            else:
                print(f"大角度右转{abs(theta_error):.1f}°>{large_angle_threshold}°")
                BoxR_turn2(1)
        
        # 第二层：前后位置调整
        elif x_error < -0.05:  # 只对明显负误差后退
            print(f"后退 x_error:{x_error:.3f}<-0.05")
            Box_Back(1)
        elif x_error > x_threshold + 0.2:
            print(f"快速前进 x_error:{x_error:.3f}>0.25")
            box_go2(1)
        elif x_error > x_threshold + 0.12:
            print(f"中速前进 x_error:{x_error:.3f}>0.17")
            box_go1(1)
        elif x_error > x_threshold + 0.06:
            print(f"慢速前进 x_error:{x_error:.3f}>0.11")
            box_go(1)
        
        # 第三层：左右平移调整
        elif y_error > y_threshold + 0.05:
            print(f"左移 y_error:{y_error:.3f}>0.08")
            BoxL_move2(1)
        elif y_error < -y_threshold - 0.05:
            print(f"右移 y_error:{y_error:.3f}<-0.08")
            BoxR_move2(1)
        elif y_error > y_threshold:
            print(f"微左移 y_error:{y_error:.3f}>0.03")
            BoxL_move1(1)
        elif y_error < -y_threshold:
            print(f"微右移 y_error:{y_error:.3f}<-0.03")
            BoxR_move1(1)
        
        # 第四层：小角度方向调整
        elif theta_error > theta_threshold:
            print(f"小左转 {theta_error:.1f}°>{theta_threshold}°")
            BoxL_turn1(1)
        elif theta_error < -theta_threshold:
            print(f"小右转 {abs(theta_error):.1f}°>{theta_threshold}°")
            BoxR_turn1(1)
